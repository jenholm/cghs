...
def logistic_u(S):
    # ~1 deep in inflation (S << Sc), ~0 after exit
    return 1.0 / (1.0 + np.exp((S - S_c) / Delta))
...
def E_of(G, S):
    ...
    term_r = Omega_r0 * fS * np.exp(-4.0 * G)
    term_m = Omega_m0 * fS * np.exp(-3.0 * G)
    term_s = Omega_S(S)
...
fS_vec = f_internal(S)
term_r = Omega_r0 * fS_vec * np.exp(-4.0 * G)
term_m = Omega_m0 * fS_vec * np.exp(-3.0 * G)
term_s = Omega_S(S)
