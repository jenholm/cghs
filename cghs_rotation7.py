...
def run_model(
    ...
    for i in range(1, T):
        ...
        if USE_RK4:
            k1v, _, _ = rhs(ti, y)
            k2v, _, _ = rhs(ti + 0.5*dt, y + 0.5*dt*k1v)
            k3v, _, _ = rhs(ti + 0.5*dt, y + 0.5*dt*k2v)
            k4v, _, _ = rhs(ti + dt, y + dt*k3v)
...
def sn_chi2(z_sn, mu_obs, mu_err, a_hist, H_hist, H0, Omega_k0=0.0, c_over_H0_Mpc=None, M_offset=0.0):
    """
